...
export class AreaStatusComponent implements OnInit {
  ...
  areaStatusForm: FormGroup;
  isForDeleteId: number;
  List: any;

  constructor(
    private _service: AreaStatusService,
  ...
  createForm() {
    this.areaStatusForm = this.fb.group({
      Id: 0,
  ...
  getList() {
    ...
    this._service
      ...
      .subscribe(
        (data) => {
          this.areaStatuss = data.Data;
          this.hasData = this.areaStatuss?.length > 0;
  ...
  careateOrEditModalPopUp(createOrUpdateModal, data?) {
    if (data?.Id) {
      // this.editId = id;
      this.areaStatusForm.patchValue({
        Id: data.Id,
        Name: data.Name,
      });
    } else {
      // this.editId = null;
      this.areaStatusForm.reset();
      this.areaStatusForm.get('Id').patchValue(0);
    }
  ...
  onSubmit() {
    if (!this.areaStatusForm.valid) {
      this._alert.error('Please provide valid information');
      return;
    }

    this._service.create(this.areaStatusForm.value).subscribe(
      (data) => {
        console.log(data);
        if (data.HasError) {
